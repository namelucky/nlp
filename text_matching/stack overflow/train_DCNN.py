...
def train_deep_model():
    ...
    model_checkpoint_path = "model_file/2018/" + 'model0.h5'
    model =   DCCNN_Model(embedding_char_matrix)
    ...
    model.fit(x=[X_train_s1, X_train_s2], y=y_train,
...
def mypredict():
    ...
    model_path = "./model_file/2018/" + 'model0.h5'
    ...
    # y_train_predict=predict(model0,X_train_s1_char,X_train_s2_char)

    # y_test_predict_df = pd.DataFrame(y_test_predict)
    # y_train_predict_df = pd.DataFrame(y_train_predict)


    # 在测试集上，真实值为y_test_binary,预测值为y_test_predict
    # save('temp/y_test_predict.pickle', y_test_predict)
    # save('temp/y_test_binary.pickle', y_test_binary)
