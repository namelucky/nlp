...
import os
import pickle
embedding_catalog = './embedding'  # 做完embedding操作，文件存放的文件夹
...
def embedding_word(f_name, train_df, test_df, train_filename,  language_deal_train_df ,type=2):
    """
    ...
    language_s1_train=language_deal_train_df['s1_title'].tolist()
    language_s2_train = language_deal_train_df['s2_title'].tolist()
    language_body1_train = language_deal_train_df['s1_body'].tolist()
    language_body2_train = language_deal_train_df['s2_body'].tolist()
    ...
    print('结束上述操作，并存储')
    s1_language_sequence = tokenizer.texts_to_sequences(language_s1_train)
    s2_language_sequence = tokenizer.texts_to_sequences(language_s2_train)

    body1_language_sequence = tokenizer.texts_to_sequences(language_body1_train)
    body2_language_sequence = tokenizer.texts_to_sequences(language_body2_train)


    # padding操作，将数组全部填充到相同的长度
    s1_language_sequences_pad = sequence.pad_sequences(s1_language_sequence, maxlen=max_sentence_length)
    s2_language_sequences_pad = sequence.pad_sequences( s2_language_sequence, maxlen=max_sentence_length)
    body1_language_sequences_pad = sequence.pad_sequences(body1_language_sequence, maxlen=max_body_length)
    body2_language_sequences_pad = sequence.pad_sequences(body2_language_sequence, maxlen=max_body_length)


    # 存储
    save(embedding_catalog + '/s1_{}_sequences_pad.pickle'.format(train_filename), s1_language_sequences_pad)
    save(embedding_catalog + '/s2_{}_sequences_pad.pickle'.format(train_filename), s2_language_sequences_pad)
    save(embedding_catalog + '/body1_{}_sequences_pad.pickle'.format(train_filename), body1_language_sequences_pad)
    save(embedding_catalog + '/body2_{}_sequences_pad.pickle'.format(train_filename), body2_language_sequences_pad)
...
if __name__ == '__main__':
    ...
    file = 'mysql'
    language_deal_train_df = pd.read_csv('./mypair_{}.csv'.format(file),
                                    names=["id", "question1", "question2", "duplicated", "s1_title", "s2_title",
                                           "s1_body", "s2_body", "maintag"], header=0, encoding='utf-8-sig',
                                    sep=',', lineterminator="\n")

    save('y_{}.pickle'.format(file), language_deal_train_df['duplicated'].tolist())
    embedding_word(os.path.join('embedding', 'word_embedding_matrix.pickle'), pre_deal_train_df,pre_deal_test_df,file,language_deal_train_df )
